...
If you are upgrading from a version that was published under the old name **litecart**, see the **[Migration Guide](/migration-from-litecart)** for step-by-step instructions covering binary, Docker, Docker Compose, Kubernetes, Homebrew, and Go module updates. Your data and database are fully compatible — no schema migration is required.
...
For detailed information on how to customize the site design and deploy it on a separate server with Nginx, see [Customization and Deployment Guide](/customization).
