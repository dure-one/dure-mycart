<script lang="ts">
  ...
  // The shop's own marks, where it has uploaded them. Absent from a payload
  // cached before branding existed and empty when nothing has been uploaded —
  // both fall back to the mark the build shipped with.
  let branding = $derived($settingsStore?.branding)
  let siteName = $derived($settingsStore?.main?.site_name || '')
</script>
...
<header class="sticky top-0 z-50 border-b-4 border-black bg-yellow-300">
  <div class="mx-auto flex min-h-20 max-w-screen-xl flex-wrap items-center gap-4 px-4 py-3 sm:px-6 lg:px-8">
    ...
    >
      <div class="flex items-center border-4 border-black bg-white px-4 py-2">
        <!--
          The shop's own mark where it has uploaded one, and the mark the build
          shipped with otherwise. The address comes from the settings API, under
          this shop's own origin, and is never composed from anything a visitor
          sent.
        -->
        {#if branding?.logo}
          <img src={branding.logo} alt={siteName} class="h-10 w-auto max-w-[14rem] object-contain" />
        {:else}
      ...
      {#if branding?.tagline}
        <span class="text-lg font-black tracking-tight text-black uppercase">{branding.tagline}</span>
      {/if}
    </a>
