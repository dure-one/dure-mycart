<script lang="ts">
  import { FormButton, FormToggle, SvgIcon } from '$lib/components'
  ...
  function updateOption(index: number, option: ProductOption) {
    ...
    if (duplicate) {
      alert(t('products.optionNameExists', { name: option.name }))
      return
    }
...
<div class="mb-6">
  <div class="mb-4 flex items-center justify-between">
    <div>
      <h3>{t('products.productVariants')}</h3>
  ...
  {#if localHasVariants}
    <div class="space-y-6">
      ...
      {#if localVariants.length > 0}
        <div>
          <div class="mb-4">
            <h3>{t('products.generatedVariants')} ({localVariants.length})</h3>
