...
<div class="relative flex h-screen overflow-hidden">
  <div class="flex h-screen w-16 flex-col justify-between border-e border-e-gray-200 bg-white">
    <div>
      <div class="inline-flex h-16 w-16 items-center justify-center">
        <a href={base || '/'} data-sveltekit-preload-data="hover" aria-label={t('common.home')}>
          <svg class="h-8 text-green-700" viewBox="0 0 28 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
    ...
    <div class="sticky right-0 bottom-0 left-0 border-t border-gray-100 bg-white px-2">
      <ul class="py-2">
        ...
        <li class="pb-2">
          <a href={`${base}/signin`} onclick={signOut} class="block" aria-label={t('common.signOut')}>
            <div
  ...
  <div class="flex flex-grow flex-col overflow-x-hidden overflow-y-auto">
    <div class="relative block w-full grow px-5 pt-5 pb-5">
      {#if children}
        {@render children()}
      {/if}
    </div>
    ...
    <div class="sticky right-0 bottom-0 left-0 flex h-12 border-t border-t-gray-200 bg-zinc-50">
      ...
      <button
        type="button"
        onclick={goToRelease}
        class="flex-none cursor-pointer border-0 p-4 text-xs {version.new
          ? 'border-gray-300 bg-yellow-200 text-gray-500'
          : 'border-gray-100 text-gray-300'}"
        aria-label={t('common.viewRelease')}
      >
        {t('common.poweredBy')}
        {#if version.new}
